...
type Config struct {
	...
	DialTimeout    time.Duration // Maximum time to try to dial a target
...
func newCslb() *cslb {
	...
	t.netDialer = &net.Dialer{ // Set up a net.Dialer identical to the
		Timeout:   2 * time.Second, // way that net.http does.
...
func NewWithConfig(cfg Config) *cslb {
	t := newCslb()
	t.Config = cfg

	if cfg.Resolver != nil {
		t.netResolver = cfg.Resolver
	}
